...
namespace PaJaMa.DatabaseStudio.Compare.Classes
{
    public class TableSynchronization : DatabaseObjectSynchronizationBase<Table>
    {
        ...
        public TableSynchronization(Database targetDatabase, Table table) : base(targetDatabase, table)
        {
        }
        ...
        private string getCreateScript()
        {
            ...
            sb.AppendLineFormat("CREATE TABLE {0}(", string.Format("{0}{1}",
                string.IsNullOrEmpty(databaseObject.Schema.SchemaName) ? string.Empty : schema + ".", tbl));
