...
" :
        ...
        public void RemoveIndexes(IDbCommand cmd)
        {
            foreach (var ix in Indexes)
            {
                cmd.CommandText = new Compare.Classes.IndexSynchronization(ParentDatabase, ix).GetRawDropText();
                cmd.ExecuteNonQuery();
        ...
        public void AddIndexes(IDbCommand cmd)
        {
            foreach (var ix in Indexes)
            {
                ...
                var items = new Compare.Classes.IndexSynchronization(ParentDatabase, ix).GetCreateItems();
        ...
        public void RemoveForeignKeys(IDbCommand cmd)
        {
            foreach (var fk in getForeignKeys())
            {
                ...
                cmd.CommandText = new Compare.Classes.ForeignKeySynchronization(ParentDatabase, fk).GetRawDropText();
                cmd.ExecuteNonQuery();
        ...
        public void AddForeignKeys(IDbCommand cmd)
        {
            foreach (var fk in getForeignKeys())
            {
                ...
                var items = new Compare.Classes.ForeignKeySynchronization(ParentDatabase, fk).GetCreateItems();
                foreach (var item in items)
                {
                    foreach (var script in item.Scripts.Where(s => s.Value.Length > 0).OrderBy(s => (int)s.Key))
                    {
